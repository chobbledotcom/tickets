...
const getListingsForSelector = async (
  ...
  const active = filter((e: ListingWithCount) => e.active)(allListings);
  ...
  const selectedInactive = filter(
    (e: ListingWithCount) => selectedIds.has(e.id) && !e.active,
  )(allListings);
...
): Map<number, ListingWithCount> => new Map(listings.map((e) => [e.id, e]));
...
const applyCreate = async (
  ...
  if (!check.ok) {
    return {
      flashError:
        "Not enough spots available for one or more selected listings — nothing was saved",
      ok: false,
    };
  }
...
const applyEdit = async (
  ...
  if (!editResult.success) {
    ...
    return {
      flashError:
        "Not enough spots remain for one of the selected listings — nothing was saved. Please review your listing lines and try again.",
      ok: false,
    };
  }
