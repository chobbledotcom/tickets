...
export const isIncompletePayment = (
  ...
  hasPaidEvent && !attendee.payment_id &&
...
const FailedPaymentRow = (
  { attendee, eventId }: { attendee: Attendee; eventId: number },
): string =>
  String(
    <tr>
      <td>{attendee.name}</td>
...
const FailedPaymentsTable = (
  { attendees, eventId }: { attendees: Attendee[]; eventId: number },
): string =>
  String(
    <table>
      <thead>
...
const DateSelector = (
  { basePath, activeFilter, dateFilter, dates }: {
    basePath: string;
    activeFilter: AttendeeFilter;
...
export const adminEventPage = ({
  ...
  const attendeesCheckedInRemaining = completeAttendees.length -
    attendeesCheckedIn;
  ...
  const checkedInClass = checkinMessage?.status === "in"
    ? "checkin-message-in"
    : "checkin-message-out";
          ...
          <table class="event-details-table">
            <tbody>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    {event.maximum_days_after === 0
                      ? "unlimited"
                      : event.maximum_days_after} days from today
                  </td>
                </tr>
              )}
              <tr>
                ...
                <td>
                  {isDaily && dateFilter
                    ? (
                      <span
                        class={completeQuantitySum >= event.max_attendees
                          ? "danger-text"
                          : ""}
                      >
                    ...
                    : (
                      <span
                        class={adjustedCount >= event.max_attendees * 0.9
                          ? "danger-text"
                          : ""}
                      >
                        {adjustedCount}
              ...
              <tr>
                ...
                <td>
                  <a href={ticketUrl}>
                    {`${allowedDomain}/ticket/${event.slug}`}
                  </a>
                  <small>
      ...
      <article>
        <aside>
          <p>
            ...
            This will permanently delete the event, all {event.attendee_count}
            {" "}
            attendee(s), any associated payment records, and all activity log
