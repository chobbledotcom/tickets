 ...
 * Returns the problem — "stage_gone" when nothing is staged at all (the
 * operator deleted the record mid-payment, the one sanctioned mid-payment
 * mutation, whose contract is that a late payment books fresh from its signed
 * order), "stage_active" when any row was already given a real quantity
 * outside this payment (the rows may be a live booking, so the caller must
 * hold the money for the operator rather than refund or re-claim), or
...
export const findStageProblem = async (
  ...
  ActivationFailure,
  "stage_active" | "stage_gone" | "stage_mismatch"
> | null> => {
  const existing = await loadExistingLines(attendeeId);
  if (existing.length === 0) return "stage_gone";
