 ...
 *
 * `linkTableSide` builds the three pieces each of them needs — read the linked
 * ids, replace the whole set, and clear the links for a key — from one
 * declaration, so the batch shape, dedupe rule, and ordering live in one place.
 *
 ...
 * direction the module uses. Table and column names are internal constants,
 * never user input. When both columns name the same kind of record — a
 * listing's children and its parents — declare the two sides together with
 * {@link selfLinkTableSides}, which answers both directions from one read.
 */
