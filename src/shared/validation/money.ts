...
const NonNegativeMoneySchema = moneyMinorSchema(0);
...
/**
 * Parse a non-negative money amount in major units (e.g. `"0"`, `"90.00"`)
 * into minor units, or `null` on the same invalid shapes as
 * {@link parsePositiveMinorUnits}. An explicit `0` is a valid result (not
 * `null`) — used by callers that accept zero as a real value, like a package
 * member's explicit free override.
 */
export const parseNonNegativeMinorUnits = (raw: string): number | null => {
  const result = v.safeParse(NonNegativeMoneySchema, raw);
  return result.success ? result.output : null;
};
