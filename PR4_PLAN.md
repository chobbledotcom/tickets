...
PR A's current implementation is in PR #2074, based on PR #2065; its production
...
| The two state columns                                            | `src/shared/db/migrations/2026-08-10_payment_state_columns.ts`, `.../schema/tables-attendees.ts`                                                                                                                                                                                                                                                                                                                                                                                             | `payment_reference_index` is a plain index because several rows may represent one charge; `protected_state` is the plaintext live-work mirror. New payment rows write the reference and index together. Before a refund snapshot is loaded, the authenticated global preparation below fills old indexes and creates missing legacy anchors. `evidence_index` remains planned and is not declared                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Every durable reference holder prepared                          | `src/shared/db/payment-reference-holders.ts`, called by `getRefundPaymentReferences` (`test/shared/db/payment-claim/shared-references.test.ts`, `test/shared/db/payment-references/storage.test.ts`)                                                                                                                                                                                                                                                                                         | Before a non-empty refund snapshot, one authenticated pass decrypts every stored reference, fills every missing blind index, and gives every row-less legacy `payment_id` a deterministic per-attendee anchor. A selected attendee can therefore see another holder of the same old charge. The current pass is intentionally global but not yet bounded or resumable; that scaling follow-up is in `TODO.md`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Durable owner-review work                                        | exact marker writers in `src/features/admin/refunds/{provider-reviews,ledger-findings,readiness-run}.ts`; owner action in `src/features/admin/attendee-payment-review.ts` and `src/shared/db/payment-review.ts` (`test/features/admin/attendee-payment-review.test.ts`)                                                                                                                                                                                                                      | Provider conflicts, unsafe partially returned obligations, and shared references are attached to the exact rows that carry them. The owner-only typed-confirmation action atomically clears review markers, or resolves a stale `unresolved` claim to a terminal outcome, and logs the decision. It never clears a fresh or capability-bound claim, and it is an acknowledgement rather than the later allocation/obligation decision                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| The operator-facing payment safety state                         | `PaymentWorkStatus` and `getPaymentWorkStatus` in `src/shared/db/payment-review.ts`; action visibility in `src/features/admin/attendee-page.ts`; refund GET/POST admission in `src/features/admin/attendee-refunds/single.ts`; exhaustive result destinations in `src/features/admin/attendee-refunds/single-result.ts`; real journeys in `specs/payments/{checking-before-a-refund,waiting-for-a-refund,recovering-the-money-record,refunding-from-two-windows,only-owners-refund}.feature` | `clear`, `moving`, `needs_money_record`, and `needs_review` are one exhaustive state shared by links, confirmation pages, and submitted forms. Moving, unrecorded, and review work remove the refund action and render no usable refund form; a copied stale form is checked again on POST. An uncertain provider answer tells the owner not to resend and points to refresh. Only owners may refund or acknowledge review work: managers see neither link, and copied owner GET and POST requests fail before provider or Money work. The Cucumber stories exercise public booking, owner and manager login, rendered forms, two real browser windows, provider delays, and ledger recovery through production routes; only provider responses and the deliberate database fault are controlled at their external boundaries                                                                                                                                                                                                                                                                                                                                                                             |
...
| Exact refund-ledger planning and results                         | `src/shared/refund-ledger/{plan,record,result}.ts` (`test/shared/refund-ledger/plan/{partial,reference-placement,whole-account}.test.ts`, `test/shared/refund-ledger/record/batch.test.ts`, `test/features/admin/refunds/provider/batch/ledger-findings.test.ts`)                                                                                                                                                                                                                            | `computeAttendeeRefund` maps each returned reference to provider-funded event groups, separates already-recorded and new reversals, and refuses unsafe groups with operator money or an unsettled obligation. `RefundLedgerResult` names exact `recorded`, `unrecorded`, and `reviewReferenceIndexes`; row settlement carries those findings back to the exact payment rows. Batch posting is atomic first and falls back per attendee after a batch failure. A returned deposit no longer cancels the sale it only part-paid: it remains unrecorded and receives `partially_returned_obligation` review                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Per-attendee release                                             | `underAttendeeClaim` in `src/features/admin/refunds/claim.ts`; attempts in `attempt.ts`; wave/ledger orchestration in `provider.ts`                                                                                                                                                                                                                                                                                                                                                          | The claim hands rows back by attendee and the run settles each independently, so one uncertain attendee does not hold settled neighbours. An unanswered call stays held. On stale resume, inherited capability is carried per attendee and exact reference: keyed references may repeat their deterministic request while keyless references only observe. A lost release remains visible as refund work, so a later run can finish it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Claimed payment refresh                                          | `refreshClaimedPayment` in `src/features/admin/refunds/refresh.ts`, through `runRefundReadiness` (`test/features/admin/refunds/refresh/{blocking,returned,review}.test.ts`, `test/features/admin/attendees-edit.test.ts`)                                                                                                                                                                                                                                                                    | Refresh now takes the same exact attendee-row claim, provider readiness, shared-reference park, per-reference inherited capability, marker writes, and ledger findings as a refund run. It never applies evidence to rows that moved or disappeared. It observes only: an inherited keyless reference with no settled proof remains held rather than being resent                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
Still promises: declared-observation sweeps, callback-scope claims, historical
marker repair, and allocation-driven shared-reference work.
...
Three boundaries remain live in the code and belong to later layers:
...
- **A shared legacy charge has no authoritative allocation.** The current path
  ...
  The owner review action only acknowledges that marker; it does not allocate
  money, and another refund attempt parks again. A later revision-fenced owner
  decision must allocate the captured Money across stable booking obligations or
  reject the automated action. No split is inferred.
- **Historical completion markers are not proof.** Before this cutover, an
  ...
  provider had not proved the full capture returned. Discovery now refuses an
  untagged returned marker as `historical_marker`; no schema member or writer
  pretends it was verified. Repair or audited owner attestation remains later
  work, and no migration may stamp those rows silently.
...
| Owner acknowledges a marker or resolves a stale unresolvable claim ("mark reviewed")                                                                                                                                                            | Admin attendee payment action (owner level, PR A)        | Typed confirmation retires current review markers — or converts a STALE claim whose capability never resolved into a terminal owner-resolved outcome — and their `protected_state` mirrors in one transaction; the decision is recorded in the activity log and deletion, merge, and retention may resume. A fresh claim or a stale keyed/keyless claim is never cleared this way. This action records acknowledgement only: it does not allocate a shared charge or decide a partially returned obligation, so unchanged shared work parks again on its next refund attempt                                                                                                                                                                                                                                                                                                                        |
...
M4 Part A's row-state slice adds exactly two columns on `processed_payments`:
`protected_state`, the plaintext live-work mirror law 1 requires for the
consumer that cannot decrypt; and `payment_reference_index`, a blind one-way
...
no longer each claim only their own row. New writes populate the index at write
time. Before any refund snapshot, `prepareRefundReferenceHolders` fills every
old row's index and creates a deterministic per-attendee anchor for every
row-less legacy reference, so there is no never-touched sibling blind spot. The
current global pass is not yet bounded or resumable; `TODO.md` carries that
scaling work. `evidence_index` remains a later-layer promise and is not declared
by Part A. The durable refund serialization needs nothing further — the staged
refund-in-flight marker rides in `failure_data`, and the provider tag rides
inside the owner-key-encrypted reference value, not another column. That slot is
ONE env-key-encrypted record. Part A uses distinct fields for the claim,
owner-review marker, unrecorded-ledger fact, and terminal outcome; the committed
observation summary remains later work. Every current writer rewrites the whole
record with the other fields preserved, conditioned on the exact value it read,
so a claim lands beside a terminal record (an operator retrying a reference
whose recorded refund failed) without clobbering it.
...
with no `processed_payments` row does not escape the claim. Authenticated
reference preparation runs before the candidate snapshot and creates one
deterministic anchor per `(attendee, reference index)`; the claim transaction
also mints any selected anchor still missing and checks the attendee PII
revision before holding it. If two attendees carry the same legacy reference,
both anchors are visible through the plain non-unique index, the claim expands
to both, and readiness parks every representation with a `shared_reference`
marker before provider I/O. If one attendee carries it, the anchor gives
concurrent runs one row to serialize on. Refresh uses the same claim and anchor
path, so a legacy partial-refund finding is durable too. This claim is what
makes the SumUp cells above real: with no idempotency parameter, two truly
...
- Exactly ONE new route, no new roles: the owner-level "mark reviewed" action on
  ...
  stale unresolved claim; acknowledgement recorded). It carries the same
  authentication as every owner admin action and is rendered only where work is
  currently reviewable. While a marker lives, the Refund action is hidden. The
  action does not allocate shared money or decide an obligation: after it clears
  a `shared_reference` marker, the unchanged multi-row representation remains
  and another refund attempt parks it again. Allocation-aware permanent gating
  belongs to the later obligation layer. The encrypted activity log carries
  conflict kinds, resource ids, and amounts; the ntfy alert carries the fixed
  error code only, per the commands table's alert boundary. Neither carries
  buyer PII or raw provider payloads (M3's fixed-refusal discipline is
  unchanged).
...
- The refresh-payment route's `refunded ? "completed" : "none"` boolean mapping
  ...
  reconciliation. Part A also ships an explicit owner acknowledgement: typed
  confirmation retires current review markers and their mirrors atomically,
  records the activity, and restores deletion, merge, and retention. It does not
  create the later allocation or obligation decision. A shared reference is
  still represented by several rows, so a later refund attempt detects it and
  parks it again. The same action may resolve a STALE claim only when its
  capability is still `unresolved`, converting it to a terminal owner-resolved
  outcome; fresh claims and stale keyed/keyless claims remain blocked because
  their calls may have moved money.
...
- Budgets: on the normal arms, 2 provider calls per admin Stripe reference and
  ...
  transaction per refund run (claim before any provider call, minting anchor
  rows for row-less legacy references; finalize/release after), counted in the
  admission's own arithmetic; everything else unchanged.
