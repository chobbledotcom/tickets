...
| The two state columns                                            | `src/shared/db/migrations/2026-08-10_payment_state_columns.ts`, `.../schema/tables-attendees.ts`                                                                                                                                                                                                              | `payment_reference_index` takes a PLAIN index, not a unique one — see the anchor-claim rules below, which the migration corrected. The finalize writes it in the same statement as the reference it indexes, so a new row can never carry one without the other; a row written before the column existed is filled in on the first authenticated refund read (`getRefundPaymentReferences`), which is the only place that can — the index is derived from the owner-key-encrypted reference, so no migration could compute it. `evidence_index` remains planned; it is not part of this landed migration                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| The claim's rules                                                | `src/shared/payment/claim.ts` (`test/shared/payment/claim.test.ts`)                                                                                                                                                                                                                                           | Pure: scope ownership, staleness against `claimLeaseMs(STALE_RESERVATION_MS)`, and evidence-derived release. The lease is the greater of reservation staleness and a five-minute request-lifetime floor. An unanswered call never releases immediately; after the lease, a keyed claim may repeat its deterministic request while a keyless claim remains observation-only until evidence settles it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| The all-or-none claim                                            | `src/shared/db/payment-claim/take.ts`, release/row mechanics in `src/shared/db/payment-claim.ts` (`test/shared/db/payment-claim/take.test.ts`, `test/shared/db/payment-claim.test.ts`, `test/features/admin/refunds/provider/claim-set-races.test.ts`)                                                        | `attendee_set` scope only — the `callback` scope's claims are PR B's. A release names the claim it took and clears only that one, so a stalled run cannot strip its successor's hold. The claim receives the exact attendee/reference snapshot the route loaded, mints any deterministic legacy anchors inside the same write transaction, and checks that every attendee's encrypted PII revision plus every `(attendee, session, reference index)` row still agrees before it writes a hold. A deletion, move, new row carrying a payment reference, changed legacy `payment_id`, or vanished loaded anchor therefore refuses the whole run before any provider read. A charge written since the index column carries its index from finalize or merge, while the first authenticated refund read repairs an older row, so two independent requested sets carrying one reference exclude each other; one requested set containing both attendees claims them together, and provider orchestration deduplicates that reference to one read and send. A legacy row nobody has loaded yet is still unindexed, and the sibling lookup can miss it until a backfill lands (`TODO.md`, "Legacy sibling rows can go unindexed until it is too late") |
...
| Each provider's refund capability                                | `refundCapability` on `PaymentProvider` and its three adapters                                                                                                                                                                                                                                                | New: each claim records the capability its provider proves, so every adapter declares it (Stripe/Square `keyed`, SumUp `keyless`). The exhaustive retry table uses that stored fact after a stale resume: keyed work may resend its deterministic request; keyless work can only observe                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| The per-provider refund evidence                                 | `ProviderRead`, `readCharge`, and `chargeMoneyRead` in `src/shared/payment/{provider-read,resources}.ts`; Stripe in `stripe-provider.ts`; Square in `square/{api,client,payment-outcomes,transport}.ts` plus `square-provider.ts`; SumUp in `sumup/{money,transaction,transport}.ts` plus `sumup-provider.ts` | Replaces `isPaymentRefunded` outright — that method and its null collapse are gone from the interface. `found`, `missing`, `unavailable`, and `invalid` are exhaustive and survive every adapter. HTTP 400/422 reads are invalid `rejected_request` evidence, 404 is missing, and retryable/auth/rate/server failures remain unavailable. Stripe reads the charge's captured amount/currency/`amount_refunded` and refuses an uncaptured state; Square reads its captured and refunded totals and only accepts `COMPLETED`; SumUp validates the transaction id, merchant, captured state, amount, currency, and every refund event                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| The per-provider refund attempt                                  | `RefundAttemptResult` / `RefundRequest` in `src/shared/payment/refund-attempt.ts`; shared boundaries in `provider-failures.ts`; provider adapters above; admin orchestration in `src/features/admin/refunds/{attempt,claim,provider,waves}.ts`                                                                | A send returns exactly `completed`, `accepted`, `rejected`, `not_sent`, or `uncertain`. Named Stripe/Square refunds must match the admitted parent, amount, currency, and documented status. SumUp's empty success is never called completion: one fresh transaction read must account for the entire capture, and a failed event or multiple pending events stay distinct. Known HTTP, timeout, network, and malformed-response failures are tagged; an unknown internal error still throws. Buyer and admin consumers record completion only for `completed` or an independently observed already-returned charge. Admin `accepted` and pre-existing in-flight refunds are tallied as pending rather than failed. Every attendee's complete reference set is read before any of its sends, provider reads and sends run in groups of at most five, and the total groups remain sequentially unbounded. Stripe and Square make exactly one fresh evidence read after an uncertain send. An inherited keyed claim may repeat its deterministic request after the lease; an inherited keyless claim is observation-only across every attendee sharing that reference until a later read proves settlement                                        |
...
| Partial reversal in the ledger                                   | `returnedRefundGroups` in `src/shared/refund-ledger.ts` (`test/features/admin/refunds/provider/batch/ledger.test.ts`)                                                                                                                                                                                         | New, and an owner decision (2026-08-11): the ledger reverses the event groups whose money came back rather than the whole account or nothing. `mapRefund` already worked one group at a time, so only the picking changed. The whole-account path is unchanged, legacy references included. `CandidateRefund` carries the references that returned, and a partial post records the money without counting the attendee refunded. Idempotency is per group: placement is decided against every original booking group before already-reversed groups are filtered, so a partial replay reports `posted:true` without another transfer or a false `unrecorded` marker. Named references must match a real provider-payment group, and legacy returns must exactly match the unnamed groups; missing or surplus money stays unplaceable while clean groups still post                                                                                                                                                                                                                                                                                                                                                                              |
...
| Per-attendee release                                             | `underAttendeeClaim` in `src/features/admin/refunds/claim.ts`; attempts in `attempt.ts`; wave/ledger orchestration in `provider.ts`                                                                                                                                                                           | New: the claim hands its rows back grouped by attendee and the run gives a verdict for each, so one uncertain attendee no longer holds their settled neighbours' rows. Settlement is per attendee, so the release is too. An unanswered call never releases immediately. On stale resume, `ClaimResult.inherited` carries the capability its own call used: keyed work may repeat the deterministic request, while keyless work is reference-wide observation-only until provider evidence settles it. A hold whose release write was lost makes its attendee a refund candidate again (`underRefundClaim` in `payment-references.ts`, read by `getRefundCandidates`): nothing else can take a hold off, so without it a refunded attendee stayed undeletable and unmergeable for good                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
Five faults are live in the code right now, and are to close rather than settled
design:

- **Only the admin refund routes claim.** Both take the hold before they read a
  provider — the bulk batch through `processRefundBatch` and the single refund
  through `attendee-refunds.ts`, each via `underAttendeeClaim`. The callback
  arms (`tryRefund` and `refundRejectedCharge` in
  `payment-processing/refunds.ts`) still send without one, so their keyless
  double-send window stays open until the `callback` scope lands. The read-only
  refresh route never sends; its current gap is an unclaimed provider read,
  marker write, and ledger write that can race a merge, delete, or refund and
  apply evidence to rows that moved or disappeared. A balance callback can no
  longer land a charge under a running refund, though:
...
- **A partial refund the refresh route observes leaves no durable mark.**
  ...
  state and `reportWithheldRefund` raises it, so no money moves wrongly and the
  operator IS told — but nothing writes the `review` marker, so the record does
  not survive the alert. Until it does, the attendee stays deletable, the old
  payment row stays prunable, and the Refund action stays on offer to rediscover
...
- **Historical SumUp completion markers are not proof.** Before this cutover, an
  accepted empty refund response or an ambiguous top-level `REFUNDED` status
  could write `provider_refunded_at` and reverse the full ledger even when the
  provider had not proved the full capture returned. The verified-marker layer
  therefore depends on provider tagging/discovery and an explicit owner choice:
  either complete in-app repair for every historical row (recommended), or
  external correction followed by an audited owner attestation. No migration may
  silently stamp those rows verified before that choice.
...
- Our durable completed-refund records: a ledger `refund_cash` leg and
  ...
  exception: before this cutover, an accepted empty refund response or an
  ambiguous top-level `REFUNDED` status could write the marker and full ledger
  reversal without proving the whole capture returned. Those old marker/ledger
  pairs must remain unverified until provider tagging/discovery and the
  owner-approved cutover repair verify or correct them.
...
5. **States never straddle slices.** The PR that first WRITES a state ships
   ...
   and PR A's money writes are what the judged-fingerprint fence guards — so PR
   A ships the whole state SCHEMA: the one migration with all three columns
   (`evidence_index`, `protected_state`, `payment_reference_index`), the marker
   vocabulary and its retirement rules, the fence, the prune gate, and the
   merge/delete admissions including the marker refusal. PR B adds no columns:
   it ships the callback-side consumers — the replay identity, committed
   evidence summaries, and the staged rejection-arm lifecycle — on a schema and
   routing that already exist whole.
...
money writes and the first marker): `evidence_index`, the replay fingerprint
above; `protected_state`, the plaintext live-work mirror law 1 requires for the
one consumer that cannot decrypt; and `payment_reference_index`, a blind one-way
index of the reference's stable identity (the `ticket_token_index` pattern) that
lets the claim transaction see, in SQL, a live claim on the SAME provider
reference held by ANOTHER row — two attendees carrying one SumUp reference can
no longer each claim their own row and race two keyless refunds: the second
claimant answers in-progress, and a sequential second run's pre-attempt read
sees the covering cumulative instead (regression: two attendees storing the same
reference, refunded concurrently, produce exactly one provider call). New writes
populate the index at write time; a legacy row gains it on first authenticated
touch, and a never-touched legacy duplicate is the F21 residual M6's backfill
closes — narrowed by this column from "any duplicate" to "duplicates never
touched since upgrade", stated. (All three columns are empty on every row that
exists today, so current pruning behavior is unchanged until a claim, staged
refund, or marker actually lands). The durable refund serialization needs
nothing further — the staged refund-in-flight marker rides in `failure_data`,
and the provider tag rides inside the owner-key-encrypted reference value, not a
third column. That slot is ONE env-key-encrypted record with a distinct field
per concern — the staged claim, the owner-review marker kind, the committed
observation summary, the terminal outcome data — and every writer rewrites the
whole record with the other fields preserved, conditioned on the exact value it
read (the same compare-and-set shape as everything below), so a claim lands
beside a terminal record (an operator retrying a reference whose recorded refund
failed) without clobbering it. An evidence-advancing write against a row under a
live FRESH claim does not land at all — it answers retryably per law 1's writer
exclusivity — because a condition checked in the database cannot govern the HTTP
call that follows it: only exclusion keeps a judged verdict valid through the
provider call. Throughout this contract, PLAN.md's atomicity law is the standing
default: facts that must agree — a marker and its mirror, a summary and its
fingerprint, a buyer record and its staged state, a retirement and its
completion — change in ONE statement, batch, or interactive transaction, and the
only fences that exist (the claim's staleness rule, the judged- fingerprint
condition on money writes) each span a gap atomicity cannot close: a provider
call or a concurrent request in the middle. Before ANY provider refund call,
every entry point — the callback arm, the admin single route, a bulk wave —
claims the reference's `processed_payments` row by compare-and-set: the staged
field written where the record it read carried NO live claim, with the batch's
written-at time and the claim's OWNER SCOPE — the callback rejection arm claims
`callback` scope for its own session; the admin single route and a bulk wave
claim `attendee_set` scope naming the claiming run's attendee — affected-rows
deciding the winner. The claim is per ATTENDEE REQUEST and all-or-none: a route
...
table's "one payout" answers rest on that claim for EVERY reference — the
row-less legacy case is minted by the claiming write above, so no reference
falls back to provider-side serialization: SumUp's cells hold on the claim
...
- Exactly ONE new route, no new roles: the owner-level "mark reviewed" action on
  an attendee's payment (typed confirmation; retires a marker or resolves a
  stale unresolvable claim; decision recorded). It ships in PR A with the first
  marker writer, carries the same authentication as every owner admin action,
...
- The refresh-payment route's `refunded ? "completed" : "none"` boolean mapping
  ...
  retention exemption in the same refresh that posts the completion). A
