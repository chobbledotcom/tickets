...
| The claim's rules                                                | `src/shared/payment/claim.ts` (`test/shared/payment/claim.test.ts`)                                                                            | Pure: scope ownership, staleness against the reservation cutoff, and capability-derived release. Staleness compares against an ISO cutoff rather than taking a clock, matching the reservation sweep                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| The all-or-none claim                                            | `src/shared/db/payment-claim.ts` (`test/shared/db/payment-claim.test.ts`)                                                                      | `attendee_set` scope only — the `callback` scope's claims are PR B's. A release names the claim it took and clears only that one, so a stalled run cannot strip its successor's hold. A charge written since the column carries its index, from the finalize or the merge anchor, and an older row is repaired by the first authenticated read that loads it — so two rows sharing one reference exclude each other. A legacy row nobody has loaded yet is still unindexed, and the sibling lookup can miss it until a backfill lands (`TODO.md`, "Legacy sibling rows can go unindexed until it is too late"). A charge held only in the attendee's `payment_id` column is given an anchor row by `payment-anchor/mint.ts` before the claim, so a run can no longer be told it holds an attendee whose money is on no row at all                                                                                                                                                                                                                                                          |
...
| Partial reversal in the ledger                                   | `returnedRefundGroups` in `src/shared/refund-ledger.ts` (`test/features/admin/refunds/provider/batch.test.ts`)                                 | New, and an owner decision (2026-08-11): the ledger reverses the event groups whose money came back rather than the whole account or nothing. `mapRefund` already worked one group at a time, so only the picking changed. The whole-account path is unchanged, legacy references included. `CandidateRefund` carries the references that returned, and a partial post records the money without counting the attendee refunded Idempotency is per group too: each reversal posts under a group derived from the booking's (`refundEventGroup`), so a charge that comes back on a later run still posts — "any refund leg at all" would have closed the account after the first partial one                                                                                                                                                                                                                                                                                                                                                                                                |
| Money the books have not caught up with                          | `unrecorded` on `PaymentRowState`; its entry in `LIVE_WORK` (`test/shared/payment/admit-move.test.ts`, `test/shared/db/payment-claim.test.ts`) | New, and the merge blocker the owner named (2026-08-11). A provider refund that the ledger could not record now says so on the row: it stops a delete, which would destroy the repair target, but not a merge, which relocates it. The mirror carries it, so the prune and the orphan purge honour it for free. It retires when a later run's ledger post lands. "Not recorded" is no longer treated as doubt — the provider answered clearly — so the claim is kept for a genuinely unconfirmed call alone. The verdict names the rows the posting actually returned, so a sibling charge still with the provider is never stamped as returned money                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Per-attendee release                                             | `claimAttendeeRows`' `held` map and `underAttendeeClaim`'s verdicts in `src/features/admin/refunds/provider.ts`                                | New: the claim hands its rows back grouped by attendee and the run gives a verdict for each, so one uncertain attendee no longer holds their settled neighbours' rows. Settlement is per attendee, so the release is too. A resumed hold is judged under the capability its OWN call was made under (`ClaimResult.inherited`), not the provider selected now — otherwise switching providers could release a keyless hold and pay the buyer twice. A hold whose release write was lost makes its attendee a refund candidate again (`underRefundClaim` in `payment-references.ts`, read by `getRefundCandidates`): nothing else can take a hold off, so without it a refunded attendee stayed undeletable and unmergeable for good                                                                                                                                                                                                                                                                                                                                                         |
