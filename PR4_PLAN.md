...
| Per-attendee release and ordered local writes                    | `underAttendeeClaim` in `src/features/admin/refunds/claim.ts`; attempts in `attempt.ts`; wave/ledger orchestration in `provider.ts` (`test/features/admin/refunds/provider/batch/write-order.test.ts`)                                                                                                                                                                                                                                                                                                                                                              | The claim hands rows back by attendee and the run settles each independently, so one uncertain attendee does not hold settled neighbours. An unanswered call stays held. On stale resume, inherited capability is carried per attendee and exact reference: keyed references may repeat their deterministic request while keyless references only observe. Within a provider wave, claim-arm and returned-marker writes share one ordered lane while provider HTTP calls retain bounded concurrency, so SQLite's single writer never races the batch against itself. A lost release remains visible as refund work, so a later run can finish it                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
