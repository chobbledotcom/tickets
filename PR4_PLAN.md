...
The whole admin command is admitted before money moves. Its worst-case physical
provider requests, database work, rollback room, settlement, and caller tail
must fit the remaining Bunny allowance. Provider requests may still overlap in
groups of five, but concurrency is not mistaken for a total budget. Once money
may have moved, returned markers, ledger findings, owner-review cases, and claim
retirement are derived from durable facts on the exact rows.
...
> loaded. A command whose complete worst case cannot fit the remaining request
> budget refuses before its first provider send. After a send is armed, the
> claim remains until durable provider and ledger facts prove what finished; a
> confirmed return whose ledger write fails becomes exact `unrecorded` work
> before settlement can release the claim.
...
| Whole-command subrequest admission                               | `src/features/admin/refunds/{budget,dispatch,readiness-run}.ts`; fixed reserves in `src/features/admin/refunds/budget.ts`, `src/shared/subrequest-budget.ts`, and `src/shared/db/client.ts` (`test/features/admin/refunds/budget.test.ts`, `test/features/admin/refunds/provider/batch/budget.test.ts`, `test/shared/subrequest-budget.test.ts`, `test/shared/db/client/transaction.test.ts`)                                                                                                                                                                       | Readiness chooses inherited holds first and admits the complete execution set before the claim and again from the row facts inside it. The budget counts each adapter's physical retry worst case, discovery, recovery reads, database work, and protected settlement/caller tails against the request's remaining database, external, and combined allowance. Dispatch prepares every candidate without sending, derives the exact provider-tagged send set, checks again before arming it, advances that set to `send_armed` once, and rechecks immediately before provider I/O. Any failed gate refuses the whole command with the audience-specific message and zero fresh provider sends. Concurrency five controls overlap only; it is not the budget                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| Per-attendee release and phase-safe failures                     | `underAttendeeClaim` in `src/features/admin/refunds/claim.ts`; attempts in `attempt.ts`; provider/ledger work in `provider.ts`; refresh persistence in `refresh.ts`; exact findings in `ledger-findings.ts` (`test/features/admin/refunds/provider/{claim,claim-completion}.test.ts`, `test/features/admin/refunds/provider/batch/{ledger-findings,write-order}.test.ts`, `test/features/admin/refunds/refresh/returned.test.ts`)                                                                                                                                   | The claim hands rows back by attendee and settles each independently, so one uncertain attendee does not hold settled neighbours. `finally` guarantees a settlement attempt, but it does not decide what is safe to release: the work records its facts first. A provider-confirmed return whose ledger call throws is placed in the exact attendee's `unrecorded` findings before the error propagates; a returned-marker failure changes the attendee to `in_doubt`, so its claim stays. Refresh follows the same ordering. Settlement then releases only attendees whose durable facts prove completion, writes exact unrecorded/review changes, and leaves every other hold visible for a later refresh. Within a provider wave, local claim/marker writes share one ordered lane while provider HTTP retains bounded concurrency                                                                                                                                                                                                                                                                                                                                                                                                                             |
