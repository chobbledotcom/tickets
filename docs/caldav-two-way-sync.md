...
| ------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Calendar URL, username, app password             | Operator-pasted, verified before saving by a live authenticated PROPFIND that must identify a **calendar collection**, then a probe event proving **write access**: a conditional create PUT (`If-None-Match: *`), a second overwriting PUT (some servers allow create but deny modify, and the mirror lives on overwrites), and a DELETE. A read-only calendar, a principal URL, or a plain WebDAV folder fails the test and is never stored. Stored as encrypted settings. Expected facts, not proof of later success. |
| The UID namespace                                | A random token minted when a calendar URL is first saved (or changed) and stored in settings. Immutable while that destination is in use, so pushed identities survive custom-domain changes, credential rotation, and disconnect/reconnect to the same calendar. Identity only — the fence against stale workers is the separate generation below.                                                                                                                                                                      |
...
| disconnected                                 | Owner saves settings (live test passes)                                              | connected. Same calendar URL as before → the dormant namespace, pushed_at stamps, and delete queue are reused, so nothing is duplicated, and pending is bumped on every listing that is dated **or still carries a pushed_at stamp** — a stamped-but-dateless row (its date was removed, then the owner disconnected before the push ran) gets its owed delete queued by the task instead of being orphaned. A different (or first) URL → stamps, queue, and status cleared, a new namespace minted, atomically; every dated listing gets pending bumped. Either way the generation is bumped — every connection transition bumps it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| connected                                    | Owner saves the same URL with new credentials                                        | Credential rotation, an explicit command: the live test runs with the submitted credentials, and on success they replace the stored ones atomically. Same destination, so the namespace, pushed_at stamps, delete queue, pending counters, and sync status all stay — nothing re-pushes, because the mirrored content did not change. The generation is bumped, fencing any straggler still working under the old credentials.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| any                                          | Database backup restored (`restoreFromSql`)                                          | Restore bypasses every transition above, so it repairs explicitly — **holding the push task's claim exactly like a settings save**, so a mid-flight pass finishes (or its lease lapses) before the wipe begins, and bumping the generation so a straggler's writes die. Before wiping, it snapshots the connection, the pushed listing ids, **and the delete queue** — a listing deleted before the restore exists only as a queue row. After replay it bumps pending on every restored listing that is dated **or carries a pushed_at stamp** (the same repair a same-URL reconnect runs), and — only when the restored destination matches the pre-restore one (same URL and namespace) — queues deletes for snapshotted pushed listings absent from the restored data **and re-inserts snapshotted queue rows the restored queue lacks**, so neither an event pushed after the backup nor one whose delete was still owed is orphaned. A different restored destination queues and re-inserts nothing; events on the other calendar stay (documented).                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Nothing                 | Probe create finds the name taken (412) | The existing resource is fetched (size-capped) and checked for our probe marker. Our own leftover from an earlier failed attempt → reused: overwritten and deleted as normal. Anything else → connect fails closed, naming the conflict — we never overwrite or delete a resource we cannot prove is our probe.                                                                                                   | Operator           |
| Probe PUTs succeeded    | Probe DELETE fails                      | Connect still fails closed — nothing stored. The probe always uses the **one fixed event name**, so a retry finds the leftover, proves it ours (above), and reuses it rather than accumulating copies; the next successful connect deletes it. The error says the test event may remain in the calendar and is safe to delete by hand.                                                                            | Operator           |
...
_which_ listing needs repair, not just that one exists). The write is inside the
task's database budget and generation-guarded; without it a scheduled-request
failure would be invisible to the operator, who only ever sees the settings
page. Sanitized means codes and counts — never raw server responses (listing
names are the operator's own site content). Connect and URL-change reset this
status to "not yet synced", and disconnect clears it, in their own transaction —
a fresh connection never wears the previous destination's success, error, or
backlog.
...
| Backup restore replays the database | A leased push pass is running   | The restore waits; a straggler past its lease writes nothing and calls nothing.                                       | The restore repair takes the same claim before wiping and bumps the generation; the self-fence stops a straggler's external calls.                                                                                                      |
...
- `vevent.ts` — pure. Build a VEVENT from a listing, sharing (not duplicating)
  ...
  vocabulary. The shared `escapeIcs` is hardened in passing to normalize `\r\n`
  and bare `\r` before escaping: today a stored description with Windows line
  endings leaks a raw carriage return into the feed's ICS output, which a CalDAV
  server may reject as malformed (regression test: CRLF input). The build uses
  the same public-visibility gate the `/ticket` page and feed use, so an event
  never links to a page that would 404. No parser for the sync path: nothing is
  read.
- `client.ts` — thin IO. `testConnection` (authenticated PROPFIND that must
  prove a calendar collection, then the probe proving write access: conditional
  create PUT, overwrite PUT, DELETE — one fixed event name, and an existing
  resource at that name is fetched and proven to be our leftover probe before it
  is ever touched), `put`, `delete` over `fetch` with basic auth, the outbound
  guards above, and subrequest counting.
...
`caldav_deletes` queue table (derived href + deleted listing id + attempt time),
and the settings keys (calendar URL, username, password, namespace, generation,
...
returns (a latent feed bug fixed in passing).
...
- Budget: ~800 source lines (grew from ~650 across review rounds 4–6); task
...
- **Pure unit (mirror-located)**: `vevent` build including escaping (with the
  ...
  pending in one action, the vacuous dateless clear, the reserved retry slots
  (one in four to the oldest attempted rows; a stream of fresh work never
  starves them), skip reasons; the lease self-fence as a pure
  elapsed-versus-margin check driven by a fake clock. These carry the mutation
  gate.
- **Integration (test db + stubbed `fetch`)**: push idempotency (crash after
  ...
  and a retry reuses the same probe href; a permanently failing row rotates
  behind fresh work, which still pushes, and appears by id and name in the sync
  status; stale settings save fails closed; 401 surfacing via durable status;
  subrequest budget counting.
...
2. **Budget**: ~800 source lines in one PR — acceptable?
