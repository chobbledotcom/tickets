...
| any                                          | Child edges or package membership change                                             | `persistListingJoins` / `writePackageMembersTx` bump pending on every affected dated listing in the same transaction. These join writes flip the `/ticket` page between live and 404 without writing the listing row, and the mirrored event's link must follow — repushing an otherwise-unchanged event is a cheap idempotent overwrite.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| any                                          | Database backup restored (`restoreFromSql`)                                          | Restore bypasses every transition above, so it repairs explicitly. Before wiping, it snapshots the connection and the pushed listing ids; after replay it bumps pending on every restored listing that is dated **or carries a pushed_at stamp** (the same repair a same-URL reconnect runs), and — only when the restored destination matches the pre-restore one (same URL and namespace) — queues deletes for snapshotted pushed listings absent from the restored data, so events pushed after the backup was taken are removed instead of orphaned. A different restored destination queues nothing; events on the other calendar stay (documented).                                                                                                                                                                                                                                                                                                            |
...
| Probe PUT succeeded     | Probe DELETE fails              | Connect still fails closed — nothing stored. The probe always uses the **one fixed event name**, so a retry overwrites the same single test event rather than accumulating copies, and the next successful connect deletes it. The error says the test event may remain in the calendar and is safe to delete by hand.                         | Operator           |
...
| Permanent failures               | A PUT the server always rejects: row stays pending, attempt-stamped to the back of the rotation, surfaced in sync status.    |
| Can one failed item block others | No — per-item isolation, and least-recently-attempted-first selection rotates repeat failures behind fresh work.             |
...
- `client.ts` — thin IO. `testConnection` (authenticated PROPFIND that must
  ...
  access — the probe uses one fixed event name, so retries overwrite the same
  test event instead of accumulating copies), `put`, `delete` over `fetch` with
  basic auth, the outbound guards above, and subrequest counting.
...
and the settings keys (calendar URL, username, password, namespace, settings
...
work instead of starving the bounded budget.
...
- Budget: ~750 source lines (grew from ~650 across review rounds 4–5); task
  budget ~4 db + ~12 external calls per wake (chunked, follow-up for more);
  connect verification is 3 foreground external calls (PROPFIND, probe PUT,
  probe DELETE) at save time only. DB calls on listing writes: zero extra
...
- **Integration (test db + stubbed `fetch`)**: push idempotency (crash after
  ...
  (or regains) its link; restore repair — restored dated-or-stamped rows
  re-mark, a pushed listing absent from the restored data gets its delete queued
  when the destination is unchanged, and nothing is queued when it differs;
  catalog-import and bulk-clone listings start pending, and pre-existing undated
  rows clear on the first wake without external calls; a settings save during a
  leased pass fails closed with the retry message; connect refuses a
  non-calendar target and a read-only collection (probe fails → nothing stored);
  probe-DELETE failure → connect fails closed and a retry reuses the same probe
  href; a permanently failing row rotates behind fresh work, which still pushes;
  stale settings save fails closed; 401 surfacing via durable status; subrequest
  budget counting.
...
2. **Budget**: ~750 source lines in one PR — acceptable?
