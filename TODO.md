...
## Close the 244 mutation survivors in the admin files that slices 2 and 3 touched

_Origin: the whole-branch mutation gate on PR #2115 (ADMIN_SURFACE_PLAN.md)._

`deno task precommit:mutation` mutates every `src/` file a branch changed.
Slices 2 and 3 replaced a path or a role with a lookup in 40 files, most of them
by one or two lines, so the gate pulled all 40 into scope. The run produced
2,197 mutants: 1,936 died, 246 survived, and 15 matched a recorded equivalent.
The score was 88.7%, below the 100% the gate demands.

Only 2 of the 246 survivors sat on a line the branch wrote. The measurement was
a cross-reference of each survivor line against
`git diff origin/main...HEAD --unified=0`. Both are closed: `entityTabRoutes`
gained a test for a one-character base path in
`test/features/admin/route-tables.test.ts`, and the `?? → ||` mutant in
`adminRecordPath` is recorded in
`scripts/mutation/equivalent-mutants/shared-a-l.txt` because a global regex
match returns `null` or a non-empty array, never an empty one. The new gate
helpers (`pageGuardFor`, `recordPageGuardFor`, `formPolicyFor`, `crudGates`)
produced no survivors at all.

The other 244 are older gaps in the same files. They are not this branch's work,
but the next branch that touches any of these files inherits them, because the
gate scopes by file and not by line. The count per file:

| File                                           | Survivors |
| ---------------------------------------------- | --------- |
| `src/features/admin/listings-edit.ts`          | 50        |
| `src/features/auth.ts`                         | 43        |
| `src/features/admin/attendees.ts`              | 28        |
| `src/features/admin/listing-page.ts`           | 26        |
| `src/features/admin/settings-statuses.ts`      | 19        |
| `src/features/admin/users.ts`                  | 18        |
| `src/features/admin/listings-uploads.ts`       | 13        |
| `src/ui/templates/admin/settings-statuses.tsx` | 11        |
| `src/ui/templates/admin/attributes.tsx`        | 5         |
| `src/features/admin/group-page.ts`             | 5         |
| `src/ui/templates/admin/logistics.tsx`         | 3         |
| `src/ui/templates/admin/groups/list.tsx`       | 3         |
| `src/features/admin/attendees-list.ts`         | 3         |
| `src/ui/templates/admin/holidays.tsx`          | 2         |
| `src/features/admin/site-content.ts`           | 2         |
| `src/features/admin/site-content-page.ts`      | 2         |
| `src/features/admin/groups.ts`                 | 2         |
| `src/features/admin/entity-write-tab.ts`       | 2         |
| `src/features/admin/site-pages-page.ts`        | 1         |
| `src/features/admin/news-page.ts`              | 1         |
| `src/features/admin/logistics-agent-page.tsx`  | 1         |
| `src/features/admin/entity-pages.ts`           | 1         |
| `src/features/admin/built-sites.ts`            | 1         |
| `src/features/admin/attendee-page.ts`          | 1         |
| `src/features/admin/api-keys.ts`               | 1         |

The 43 in `src/features/auth.ts` are all in the session, API key, and CSRF body
plumbing, which slices 2 and 3 never opened. The heavy `listings-edit.ts` and
`attendees.ts` counts are mostly form parse branches and redirect strings that
no test reads back.

Take the files one at a time, hardest first, and give each its own small pull
request. Run `deno task mutation <source> <mirror-test> --harness` for one file.
Read each survivor as `file:line:col old → new`, then write the assertion that
tells the two apart. Record a mutant in `scripts/mutation/equivalent-mutants/`
only with a proof that no input can distinguish it. Expect the whole job to need
several hours per large file.
