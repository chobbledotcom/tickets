...
_Origin: CodeRabbit review on PR #1845 (`src/shared/db/migrations.ts`)._
...
- **`test/features/admin/settings-wallets.test.ts`** — the four
  ...
  Email", "requires private key on initial setup", "rejects invalid PEM private
  key") repeat the same login → POST → flash-redirect scaffolding, differing
  only in the blanked field and expected message. Fold them into one
  table-driven test, following the "advanced redirect" cases in
...
- **`test/ui/client/admin/address-lookup/client.test.ts` +
  ...
  helper under `#test-utils` (or a sibling non-test file in the same folder) and
  import it from both.
...
_Origin: a new encryption test showed up in the slow-test report (PR #1945)._
...
So a single 64KB comparison costs more than the whole 500ms slow-test threshold,
and even a 64-byte one is not free. `@std/assert`'s `equal` gives the same
answer about 1,400 times faster, and comparing the two arrays' base64 form is
faster still and just as exact.
...
`test/shared/crypto/aes-gcm.test.ts` has a local `expectSameBytes` showing the
base64 approach. Applying it there took that file from 3,690ms to 39ms.
