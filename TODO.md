...
## Fix four documentation links that name functions which no longer exist

A tree-wide scan of `{@link X}` targets in `src/` found 4 of 523 naming
something that exists nowhere in the codebase, so the reader is sent to a
function they cannot find:

| File                                      | Line | Dead name            |
| ----------------------------------------- | ---- | -------------------- |
| `src/ui/templates/admin/site-content.tsx` | 53   | `EditForm`           |
| `src/features/admin/groups.ts`            | 263  | `handleGroupEditGet` |
| `src/features/admin/api-listing-joins.ts` | 58   | `writeChildEdges`    |
| `src/features/admin/listing-page.ts`      | 81   | `ListingActionNav`   |

Found while trimming `src/shared/db/client.ts` for PR #2083's follow-up, which
fixed the fifth (`syncListingPrices`, whose real name is
`syncListingPricesForIds`). Left out of that PR because each one needs its own
look at the surrounding area to find the true referent, and the four sit in
unrelated features — guessing a plausible-looking name would just replace a dead
link with a wrong one.

Starting point: the scan is a one-liner worth keeping —

```bash
for f in $(grep -rl '{@link' src/ --include=*.ts --include=*.tsx); do
  for n in $(grep -o '{@link [A-Za-z_][A-Za-z0-9_]*' "$f" | sed 's/{@link //' | sort -u); do
    [ "$(grep -rn "\b$n\b" src/ --include=*.ts --include=*.tsx | grep -vc '{@link')" = 0 ] &&
      echo "STALE $f -> $n"
  done
done
```

Read each comment, find what it actually meant to point at, and fix or drop the
link. Worth considering afterwards whether this belongs in
`deno task check:comments` as a real rule: unlike comment length, a dangling
`{@link}` is mechanically checkable, and it is the one comment defect a machine
can catch. `deno doc` may already resolve these — check whether it can be made
to fail the build instead of hand-rolling the scan.
