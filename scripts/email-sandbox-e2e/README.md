...
A leg passes when the provider accepts both requests. `sendBulkEmails` reads
Postmark's per-message results, because its batch endpoint can answer 200 and
still refuse a message inside it, so the harness reports what the production
code found. The refusal body is reported when a request fails. Each leg has a
two-minute allowance. A provider that stalls becomes a failed leg instead of a
killed job, and the harness cancels the request it left in flight.
