 ...
 *
 * Mutation-tests every staged `src/` file against every staged `test/` file and
 * demands a 100% kill rate (known-equivalent mutants recorded in
 * scripts/mutation/equivalent-mutants.txt aside). See ./precommit/mutation-step.ts
 * for the rationale; the source→test mapping is simply the staged set, because
 * the project's 100%-coverage rule lands a src change with its covering test in
 * the same commit.
 *
...
/** git yields repo-relative paths; the runner's ignore-list matching needs the
 *  absolute form so its `rel()` recovers the repo-relative key. */
