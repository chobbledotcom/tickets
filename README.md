...
**Backups:** the Backups admin page and the automatic pre-migration backup dump every table to a single `.zip`. Table reads are keyset-paginated so no single response trips libsqld's "Response is too large" payload cap (the server limit behind Bunny's databases). The _inline_ pre-migration backup runs inside one edge request, which has a limited outbound-subrequest budget, so for a database above `BACKUP_MAX_INLINE_ROWS` total rows (default 20,000) it is skipped — the migration still proceeds and the operator is alerted to take an out-of-band backup with `deno task backup`, which runs in a full Deno process with no such budget. Tune `BACKUP_PAGE_SIZE` (default 500 rows per read) and `BACKUP_MAX_INLINE_ROWS` via env if needed. A manually-triggered GitHub Actions workflow (`.github/workflows/backup.yml`) runs `deno task backup` from CI using `DB_URL`, `DB_TOKEN`, `STORAGE_ZONE_NAME` and `STORAGE_ZONE_KEY` repository secrets.
