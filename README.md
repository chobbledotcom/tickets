...
- Configurable contact fields: email only, phone only, or both
...
- Automatic refund if capacity exceeded after payment
...
- Settings: payment provider config, embed host restrictions, password change, database reset

### Encryption
- **Hybrid RSA-OAEP + AES-256-GCM** for attendee PII (name, email, phone)
...
- Payload: name, email, phone, amount, currency, payment ID, ticket URL, per-ticket details
