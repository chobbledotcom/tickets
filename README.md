...
**Backups:** the Backups admin page and the automatic pre-migration backup dump every table to a single `.zip`. Table reads are keyset-paginated so no single response trips libsqld's "Response is too large" payload cap (the server limit behind Bunny's databases). The _inline_ pre-migration backup runs inside one edge request, which can make at most [50 outbound subrequests](https://docs.bunny.net/scripting/limits). Dumping costs roughly one subrequest per table (≈31) plus one per `BACKUP_PAGE_SIZE` rows, and the migration itself needs some too — so the inline backup is skipped unless its estimate plus a reserved headroom for the migration fits within `BACKUP_MAX_INLINE_SUBREQUESTS` (default 50, Bunny's cap); in practice only small databases qualify. On a skip the migration still proceeds and the operator is alerted to take an out-of-band backup with `deno task backup`, which runs in a full Deno process with no such budget. Tune `BACKUP_PAGE_SIZE` (default 500 rows per read) and `BACKUP_MAX_INLINE_SUBREQUESTS` via env if needed. A manually-triggered GitHub Actions workflow (`.github/workflows/backup.yml`) runs `deno task backup` from CI using `DB_URL`, `DB_TOKEN`, `STORAGE_ZONE_NAME` and `STORAGE_ZONE_KEY` repository secrets.
