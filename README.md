...
**Restoring to a point in time:** the running build records the git commit *and* version it was built from into its own `settings` table (`current_script_commit` / `current_script_version`, written on boot; the commit is cleared if a build ships without one, so it never goes stale), so every dump carries what the site was running when the backup was taken. Restoring a backup rolls back the **data** and surfaces that commit and version; to roll the **code** back to match, run the **`.github/workflows/restore-deploy.yml`** workflow with that commit, version, and the target script id. It builds the edge bundle fresh from our repository at that commit (git is the per-commit source of truth) and deploys it — so a restore only ever redeploys a commit from our own history, never code carried inside an (untrusted) uploaded backup file. Passing the version stamps the rebuilt code with its original build time so the upgrade page still offers later releases. For safety the workflow validates the commit is a full SHA and deploys via a pinned external action, so the production API key is never handed to a deploy wrapper loaded from the checked-out (possibly old) commit.
