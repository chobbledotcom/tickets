...
**Restoring to a point in time:** the running build records the git commit it was built from into its own `settings` table (`current_script_commit`, written on boot), so every dump carries the commit the site was running when the backup was taken. Restoring a backup rolls back the **data** and surfaces that commit; to roll the **code** back to match, run the **`.github/workflows/restore-deploy.yml`** workflow with that commit and the target script id. It builds the edge bundle fresh from our repository at that commit (git is the per-commit source of truth) and deploys it — so a restore only ever redeploys a commit from our own history, never code carried inside an (untrusted) uploaded backup file.
