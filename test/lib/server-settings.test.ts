...
describe("server (admin settings)", () => {
  ...
  describe("POST /admin/settings/stripe", () => {
    ...
    test("settings page shows mismatch warning for test key with live publishable key", async () => {
      await withMocks(
        () =>
          stub(stripeApi, "setupWebhookEndpoint", () =>
            Promise.resolve({
        ...
        async () => {
          await handleRequest(
            mockFormRequest(
              "/admin/settings/stripe",
              {
                stripe_secret_key: "sk_test_mismatch_check",
                csrf_token: await testCsrfToken(),
              },
              await testCookie(),
            ),
          ...
          Deno.env.set("STRIPE_PUBLISHABLE_KEY", "pk_live_mismatch");
          ...
          try {
            ...
            const html = await response.text();
            expect(html).toContain("Key mismatch:");
            expect(html).toContain("STRIPE_PUBLISHABLE_KEY");
            expect(html).toContain("live");
          } finally {
            Deno.env.delete("STRIPE_PUBLISHABLE_KEY");
          }
        },
      );
    });

    test("settings page shows mismatch warning for live key with test publishable key", async () => {
      await withMocks(
        () =>
          stub(stripeApi, "setupWebhookEndpoint", () =>
            Promise.resolve({
        ...
        async () => {
          await handleRequest(
            mockFormRequest(
              "/admin/settings/stripe",
              {
                stripe_secret_key: "sk_live_mismatch_check",
                csrf_token: await testCsrfToken(),
              },
              await testCookie(),
            ),
          ...
          Deno.env.set("STRIPE_PUBLISHABLE_KEY", "pk_test_mismatch");
