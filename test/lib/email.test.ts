  ...
  describe("sendEmail", () => {
    ...
    test("returns status code on non-OK response", async () => {
      ...
      await withErrorSpy(async (errorSpy) => {
        ...
        expect(
          logs.some((l) =>
            l.includes("E_EMAIL_SEND") && l.includes("status=500")
          ),
        ).toBe(true);
      });
    });
    ...
    test("returns undefined on fetch failure", async () => {
      ...
      await withErrorSpy(async (errorSpy) => {
        ...
        expect(
          logs.some((l) =>
            l.includes("E_EMAIL_SEND") && l.includes("Network error")
          ),
        ).toBe(true);
      });
    });
    ...
    test("returns undefined for non-Error thrown values", async () => {
      ...
      await withErrorSpy(async (errorSpy) => {
        ...
        expect(
          logs.some((l) =>
            l.includes("E_EMAIL_SEND") && l.includes("string error")
          ),
        ).toBe(true);
      });
    });
    ...
    test("returns undefined for unknown provider", async () => {
      await withErrorSpy(async (errorSpy) => {
        ...
        expect(
          logs.some((l) =>
            l.includes("E_EMAIL_SEND") && l.includes("unknown provider")
          ),
        ).toBe(true);
      });
    });
