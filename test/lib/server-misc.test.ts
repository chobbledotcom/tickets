...
describeWithEnv("server (misc)", { db: true }, () => {
  ...
  describe("routes/admin/utils.ts (verifyOrRedirect)", () => {
    test("returns null when confirmation matches (trimmed, case-insensitive)", async () => {
      const { verifyOrRedirect } = await import("#routes/admin/utils.ts");
      const { FormParams } = await import("#routes/utils.ts");
      const form = new FormParams("confirm_identifier=%20TeSt%20");
    ...
    test("returns flash error redirect that includes action label in flash message", async () => {
      const { verifyOrRedirect } = await import("#routes/admin/utils.ts");
      const { FormParams } = await import("#routes/utils.ts");
