...
describe("server (webhooks)", () => {
  ...
  describe("closes_at in payment processing", () => {
    ...
    test("charge.refunded webhook marks attendees as refunded", async () => {
      await setupStripe();

      const event = await createTestEvent({ maxAttendees: 50, unitPrice: 500 });
      const result = await createAttendeeAtomic({
        eventId: event.id,
        name: "Refund WH",
        email: "refundwh@example.com",
        paymentId: "pi_refund_wh_test",
        quantity: 1,
        pricePaid: 500,
      });
      ...
      mockVerify.mockResolvedValue({
        ...
        event: {
          ...
          data: {
            object: {
              id: "ch_refund_test",
              payment_intent: "pi_refund_wh_test",
              refunded: true,
              amount_refunded: 500,
            },
          },
      ...
      try {
        const response = await handleRequest(
          mockWebhookRequest({}, { "stripe-signature": "sig_refund" }),
        );
        expect(response.status).toBe(200);
        const json = await response.json();
    ...
    test("charge.refunded webhook acknowledges when no matching attendees found", async () => {
      await setupStripe();
      ...
      try {
        const response = await handleRequest(
          mockWebhookRequest({}, { "stripe-signature": "sig_refund_nomatch" }),
        );
        expect(response.status).toBe(200);
        const json = await response.json();
        expect(json.status).toBe("no_matching_attendees");
      } finally {
        mockVerify.mockRestore();
      }
    });

    test("charge.refunded webhook acknowledges when no payment_intent in event", async () => {
      await setupStripe();
      ...
      try {
        const response = await handleRequest(
          mockWebhookRequest({}, { "stripe-signature": "sig_refund_nopi" }),
        );
        expect(response.status).toBe(200);
        const json = await response.json();
        expect(json.status).toBe("no_payment_intent");
      } finally {
        mockVerify.mockRestore();
      }
    });
