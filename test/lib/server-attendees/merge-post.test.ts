...
import { getAttendee, getAttendeeRaw } from "#shared/db/attendees.ts";
...
describeWithEnv("server (admin attendees) > merge post", { db: true }, () => {
  describe("POST /admin/attendees/:attendeeId/merge", () => {
    ...
    test("takes source PII fields when selected", async () => {
      ...
      expect(response.status).toBe(302);
      // The kept name is the source's, so the flash reads "into John Smith".
      expectFlash(
        response,
      ...
      // The stored (decrypted) record carries the adopted name too.
