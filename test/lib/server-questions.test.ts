...
describeWithEnv("server (admin questions)", { db: true }, () => {
  ...
  describe("POST /admin/questions/:id/answers", () => {
    ...
    test("adds answer with a price modifier", async () => {
      const id = await createQuestion("Choose one");
      const { response } = await adminFormPost(
    ...
    test("rejects invalid answer price modifier values", async () => {
      const id = await createQuestion("Choose one");
      const { response } = await adminFormPost(
        `/admin/questions/${id}/answers`,
        {
      ...
      expect(response.status).toBe(302);
      expectFlash(response, "Amount must be greater than 0", false);
    });

    test("rejects invalid answer price modifier options", async () => {
      const id = await createQuestion("Choose one");
      const { response } = await adminFormPost(
        `/admin/questions/${id}/answers`,
        {
      ...
      expect(response.status).toBe(302);
      expectFlash(response, "Invalid price modifier", false);
    });

    test("rejects non-numeric answer price modifier values", async () => {
      const id = await createQuestion("Choose one");
      const { response } = await adminFormPost(
        `/admin/questions/${id}/answers`,
        {
      ...
      expect(response.status).toBe(302);
      expectFlash(response, "Enter a valid number", false);
    });
