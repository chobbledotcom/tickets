...
import { makeOpts, makeRow, zaraAliceRows } from "./shared.ts";

const render = (options: AttendeeTableOptions): string =>
  String(AttendeeTable(options));
...
describe("AttendeeTable component", () => {
  ...
  test("sorts rows unless the caller marks them as presorted", () => {
    ...
    expect(presorted.indexOf("Zara")).toBeLessThan(presorted.indexOf("Alice"));
  });
  ...
  test("uses custom and translated empty messages", () => {
    ...
    const custom = render(
      makeOpts({ columnLayout: layout, emptyMessage: "Pick a day", rows: [] }),
    );
