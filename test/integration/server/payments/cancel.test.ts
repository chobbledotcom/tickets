...
describeWithEnv("server (payment flow)", { db: true, triggers: true }, () => {
  describe("GET /payment/cancel", () => {
    test("returns error for missing session_id", async () => {
      ...
      try {
        ...
        expect(
          ...
          'E_PAYMENT_SESSION detail="Payment callback missing session_id parameter"',
        );
      } finally {
        errorLog.restore();
      }
      ...
      await withMocks(
        ...
        async () => {
          ...
          try {
            ...
            expect(
              errorLog.calls.map((call) => call.args.join(" ")).join("\n"),
            ).toContain("[cancel] Session not found (session=cs_invalid)");
          } finally {
            errorLog.restore();
          }
        },
    ...
    test("closes and purges the matching staged checkout", async () => {
      await setupStripe();
      const listing = await createTestListing({ maxAttendees: 50 });
      const attendeeId = await insertOrphanAttendee(new Date().toISOString());
      await insertCheckoutStage(attendeeId, "cs_staged_cancel", {
        providerCheckoutId: "provider-staged-cancel",
      });
      ...
      await withMocks(
        ...
        async ({ close }) => {
          await assertPublicHtml(
            "/payment/cancel?session_id=cs_staged_cancel",
            "Payment Cancelled",
          );
