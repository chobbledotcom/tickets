...
import { authFailure } from "#routes/utils.ts";
...
describe("authFailure", () => {
  ...
  describe("json channel", () => {
    test("not-authenticated returns 401 JSON error", async () => {
      ...
      expect(body).toEqual({ status: "error", message: "Not authenticated" });
    });
    ...
    test("invalid-api-key returns 401 JSON error", async () => {
      ...
      expect(res.status).toBe(401);
      const body = await res.json();
      expect(body).toEqual({ status: "error", message: "Invalid API key" });
    });
  });
});
