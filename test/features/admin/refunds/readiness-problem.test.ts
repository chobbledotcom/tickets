...
describe("refund readiness messages", () => {
  ...
  for (
    const [readiness, message] of [
      [
        { kind: "not_ready", observations: [], reason: "claim_changed" },
      ...
      readonly [Extract<RefundReadinessResult, { kind: "not_ready" }>, string]
    >
  ) {
    test(
      readiness.reason,
      () => expect(refundReadinessMessage(readiness)).toBe(message),
    );
  }
});
