...
describeWithEnv(
  "admin refund provider readiness integration",
  { db: true },
  () => {
    ...
    test("keeps equal raw references distinct by provider without resending a return", async () => {
      ...
      const writes = recordingWrites();
      await markProviderRefundsReturned([squareRef], "due");
      ...
      expect(
        ...
        await paymentReferenceIndex(stripeRef),
        await paymentReferenceIndex(squareRef),
      ]);
