...
describe("paidPricingRefund", () => {
  ...
  for (const { amount, quantity } of [
    { amount: 999, quantity: 1 },
    { amount: 1001, quantity: 1 },
    { amount: 999, quantity: 2 },
  ]) {
    test(`rejects a fixed item total of ${amount} at quantity ${quantity}`, () => {
      expect(
        paidPricingRefund(
          [validated({ item: { e: 7, p: amount, q: quantity } })],
          standardOrder(),
          1000,
        ),
      ).toEqual(
