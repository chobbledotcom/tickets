...
describeWithEnv("server (payment flow: ticket success)", { db: true }, () => {
  describe("GET /payment/success (ticket)", () => {
    ...
    test("renders the signed thank-you URL with its exact ticket link", async () => {
      await setupStripe();
          ...
          metadata: signMeta(
            {
              ...
              name: "Thanks Buyer",
              thank_you_url: "https://example.com/signed-thanks",
            },
            500,
          ),
          payment_intent: "pi_signed_thanks",
          payment_status: "paid",
        } as unknown as Awaited<
          ReturnType<typeof stripeApi.retrieveCheckoutSession>
        >),
      ...
      try {
        await stageStripeCallback("cs_signed_thanks");
        const response = await handleRequest(
          mockRequest("/payment/success?session_id=cs_signed_thanks"),
        );
        const html = await expectHtmlResponse(
          response,
          200,
          "https://example.com/signed-thanks",
        );
        expect(html).toMatch(/href="\/t\/[A-F0-9]+"/);
