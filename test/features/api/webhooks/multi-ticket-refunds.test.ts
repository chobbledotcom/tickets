  ...
  () => {
    ...
    test("multi-ticket webhook removes and refunds when capacity is exceeded", async () => {
      ...
      try {
        ...
        const errors = errorLog.calls
          .map((call) => call.args.join(" "))
          .join("\n");
        expect(errors).toContain(`listing=${listing1.id}`);
        expect(errors).not.toContain("couldn't complete your booking");
        const debug = debugLog.calls
          .map((call) => call.args.join(" "))
          .join("\n");
        expect(debug).toContain("Failed payload:");
      } finally {
        debugLog.restore();
        errorLog.restore();
      }
