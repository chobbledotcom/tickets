...
test("maps every used Stripe operation to its endpoint", async () => {
  ...
  await client.checkout.sessions.expire("cs/1");
  ...
  expect(requests).toEqual([
    ...
    { body: "", method: "GET", path: "/v1/checkout/sessions/cs%2F1" },
    {
      body: "",
      method: "POST",
      path: "/v1/checkout/sessions/cs%2F1/expire",
    },
    {
      body: "",
      method: "GET",
    ...
    {
      body: "api_version=2026-04-22.dahlia&enabled_events[0]=checkout.session.completed&enabled_events[1]=checkout.session.expired&url=https%3A%2F%2Fexample.com%2Fpayment%2Fwebhook",
      method: "POST",
