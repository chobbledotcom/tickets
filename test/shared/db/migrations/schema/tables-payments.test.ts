...
describeWithEnv("db > payment aggregate constraints", { db: true }, () => {
  ...
  test("allows checkout creation data only on an unattached created session", async () => {
    ...
    await expect(
      getDb().execute(
        "UPDATE payment_sessions SET state = 'pending' WHERE id = ?",
        [PAYMENT_ID],
      ),
    ).rejects.toThrow("CHECK constraint failed");
  });
  ...
  test("does not recreate retired payment source tables", async () => {
    const result = await getDb().execute(`SELECT name FROM sqlite_master
      WHERE type = 'table'
