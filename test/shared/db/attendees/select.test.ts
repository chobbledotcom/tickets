...
describe("attendeeColumns", () => {
  ...
  test("each ledger field emits a fixed number of subqueries", () => {
    ...
    expect(subqueries("refunded")).toBe(7);
...
describe("attendee SELECT — exact generated SQL", () => {
  ...
  test("the full field set produces the exact column list (all three ledger subqueries inline)", () => {
    expect(attendeeColumns("inner", ATTENDEE_FIELDS)).toBe(
      "attendee.id, attendee.created, attendee.kind, attendee.ticket_token_index, attendee.pii_blob, attendee.status_id, attendee.split_logistics_agents, listingAttendee.listing_id, SUBSTR(listingAttendee.start_at, 1, 10) as date, listingAttendee.quantity, listingAttendee.checked_in, -(SELECT COALESCE(SUM(CASE WHEN dest_type = 'attendee' AND dest_id = CAST(attendee.id AS TEXT) THEN amount WHEN source_type = 'attendee' AND source_id = CAST(attendee.id AS TEXT) THEN -amount ELSE 0 END), 0) FROM transfers WHERE dest_type = 'attendee' AND dest_id = CAST(attendee.id AS TEXT) OR source_type = 'attendee' AND source_id = CAST(attendee.id AS TEXT)) AS remaining_balance, (SELECT CASE WHEN EXISTS(SELECT 1 FROM transfers WHERE kind = 'sale' AND source_type = 'attendee' AND source_id = CAST(listingAttendee.attendee_id AS TEXT) AND dest_type = 'revenue' AND dest_id = CAST(listingAttendee.listing_id AS TEXT) AND event_group = listingAttendee.ledger_event_group) THEN EXISTS(SELECT 1 FROM transfers WHERE kind = 'refund_sale' AND source_type = 'revenue' AND source_id = CAST(listingAttendee.listing_id AS TEXT) AND dest_type = 'attendee' AND dest_id = CAST(listingAttendee.attendee_id AS TEXT) AND reverses_group = listingAttendee.ledger_event_group) WHEN listingAttendee.quantity = 0 THEN EXISTS(SELECT 1 FROM transfers WHERE kind = 'refund_cash' AND source_type = 'attendee' AND source_id = CAST(listingAttendee.attendee_id AS TEXT)) WHEN listingAttendee.ledger_event_group = '' THEN 0 ELSE EXISTS(SELECT 1 FROM transfers WHERE kind GLOB 'refund_*' AND reverses_group = listingAttendee.ledger_event_group) AND NOT EXISTS(SELECT 1 FROM transfers AS unreturned WHERE kind = 'sale' AND source_type = 'attendee' AND source_id = CAST(listingAttendee.attendee_id AS TEXT) AND unreturned.dest_type = 'revenue' AND unreturned.event_group = listingAttendee.ledger_event_group AND NOT EXISTS(SELECT 1 FROM transfers AS returned WHERE returned.kind = 'refund_sale' AND source_type = 'revenue' AND source_id = CAST(unreturned.dest_id AS TEXT) AND dest_type = 'attendee' AND dest_id = CAST(listingAttendee.attendee_id AS TEXT) AND returned.reverses_group = listingAttendee.ledger_event_group)) END) AS refunded, COALESCE(CAST((SELECT COALESCE(SUM(amount), 0) FROM transfers WHERE kind = 'sale' AND source_type = 'attendee' AND source_id = CAST(listingAttendee.attendee_id AS TEXT) AND dest_type = 'revenue' AND dest_id = CAST(listingAttendee.listing_id AS TEXT) AND event_group = listingAttendee.ledger_event_group) * (SELECT COALESCE(SUM(sibling.quantity), 0) FROM listing_attendees AS sibling WHERE sibling.attendee_id = listingAttendee.attendee_id AND sibling.listing_id = listingAttendee.listing_id AND sibling.ledger_event_group = listingAttendee.ledger_event_group AND sibling.id <= listingAttendee.id) / NULLIF((SELECT COALESCE(SUM(sibling.quantity), 0) FROM listing_attendees AS sibling WHERE sibling.attendee_id = listingAttendee.attendee_id AND sibling.listing_id = listingAttendee.listing_id AND sibling.ledger_event_group = listingAttendee.ledger_event_group), 0) AS INTEGER) - CAST((SELECT COALESCE(SUM(amount), 0) FROM transfers WHERE kind = 'sale' AND source_type = 'attendee' AND source_id = CAST(listingAttendee.attendee_id AS TEXT) AND dest_type = 'revenue' AND dest_id = CAST(listingAttendee.listing_id AS TEXT) AND event_group = listingAttendee.ledger_event_group) * (SELECT COALESCE(SUM(sibling.quantity), 0) FROM listing_attendees AS sibling WHERE sibling.attendee_id = listingAttendee.attendee_id AND sibling.listing_id = listingAttendee.listing_id AND sibling.ledger_event_group = listingAttendee.ledger_event_group AND sibling.id < listingAttendee.id) / NULLIF((SELECT COALESCE(SUM(sibling.quantity), 0) FROM listing_attendees AS sibling WHERE sibling.attendee_id = listingAttendee.attendee_id AND sibling.listing_id = listingAttendee.listing_id AND sibling.ledger_event_group = listingAttendee.ledger_event_group), 0) AS INTEGER), 0) AS price_paid, SUBSTR(listingAttendee.end_at, 1, 10) as end_date, listingAttendee.attachment_downloads, listingAttendee.package_group_id",
    );
  });

  test("an empty field set under a LEFT join produces the exact core columns", () => {
    expect(attendeeColumns("left", [])).toBe(
      "attendee.id, attendee.created, attendee.kind, attendee.ticket_token_index, attendee.pii_blob, attendee.status_id, attendee.split_logistics_agents, COALESCE(listingAttendee.listing_id, 0) as listing_id, SUBSTR(listingAttendee.start_at, 1, 10) as date, COALESCE(listingAttendee.quantity, 0) as quantity, COALESCE(listingAttendee.checked_in, 0) as checked_in",
    );
  });
