...
describeWithEnv("db > listing-parents", { db: true }, () => {
  ...
  describe("setChildIds / getChildIds", () => {
    ...
    test("refuses a child packaged after edge validation", async () => {
      ...
      expect(
        await assignListingsToGroup([childA.id], packageGroup.id),
      ).toBeNull();

      expect(
        await withTransaction((tx) =>
          setListingChildrenWithPackageCheckTx(tx, parent.id, [childA.id]),
        ),
    ...
    test("throws the package rule from the transaction writer", async () => {
      ...
      await expect(
        withTransaction(async (tx) =>
          requireListingChildrenPackageCheck(
            await setListingChildrenWithPackageCheckTx(tx, parent.id, [
              childA.id,
            ]),
          ),
        ),
