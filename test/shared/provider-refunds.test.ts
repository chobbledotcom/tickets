...
const storedState = (id: number): Promise<StoredState | null> =>
  queryOne<StoredState>(
    `SELECT reference_index, refund_revision, refund_state, refunded_amount
       FROM payment_charges
      WHERE id = ?`,
    [id],
  );

describeWithEnv("provider refund owner cases", { db: true }, () => {
  test("includes a ready refund intent in the reachable owner queue", async () => {
    ...
    expect(await loadProviderRefundCase(id, await getTestPrivateKey()))
  ...
  test("exposes returned money until the owner confirms it is recorded", async () => {
    ...
    expect(readRefundAuthorityState(row.refund_state, "recorded case"))
  ...
  test("records returned money through one revision-fenced transition", async () => {
    ...
    const referenceIndex = (await storedState(id))!.reference_index;
    await getDb().execute({
      args: [`owner-case-${crypto.randomUUID()}`, referenceIndex],
    ...
    expect(readRefundAuthorityState(row.refund_state, "resolved case"))
      .toMatchObject({
        kind: "completed",
        local: { kind: "due" },
        proof: "owner",
    ...
    ).toMatchObject({ provider_refunded_at: expect.stringMatching(/.+/u) });
  });
  ...
  test("not-sent authorizes a new generation without sending it", async () => {
    ...
    expect(readRefundAuthorityState(row.refund_state, "rearmed case"))
      .toMatchObject({
        kind: "ready",
  ...
  test("a keyed not-sent decision preserves capability and starts a fresh window", async () => {
    ...
    expect(
      readRefundAuthorityState(
        (await storedState(id))!.refund_state,
        "keyed rearmed case",
      ),
    ).toMatchObject({
      kind: "ready",
