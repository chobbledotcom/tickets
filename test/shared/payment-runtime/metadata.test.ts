...
import { bookingIntentFromMetadata } from "#shared/payment-runtime/metadata.ts";
...
const metadata = (changes: Partial<SessionMetadata> = {}): SessionMetadata => ({
  ...bareMetadata,
  ...changes,
});

describe("reading a booking out of checkout metadata", () => {
  test("reads the lines, the buyer, and the empty fields as nothing", () => {
    expect(bookingIntentFromMetadata(metadata())).toEqual({
      address: "",
  ...
  test("reads everything the checkout can carry", () => {
    const intent = bookingIntentFromMetadata(
      metadata({
  ...
  test("reads the modifiers the buyer took", () => {
    expect(
      bookingIntentFromMetadata(
        metadata({ modifiers: JSON.stringify([{ i: 3, q: 2 }]) }),
      )?.modifiers,
    ).toEqual([{ i: 3, q: 2 }]);
  });
  ...
  for (const [name, dayCount] of [
    ...
    test(`reads no booking when the day count is ${name}`, () => {
      expect(bookingIntentFromMetadata(metadata({ day_count: dayCount }))).toBe(
        null,
      );
    });
  }
  ...
  test("keeps a day count of one", () => {
    expect(
      bookingIntentFromMetadata(metadata({ day_count: "1" }))?.dayCount,
    ).toBe(1);
  });
  ...
  for (const [name, items] of [
    ...
    test(`reads no booking when what was bought ${name}`, () => {
      expect(bookingIntentFromMetadata(metadata({ items }))).toBe(null);
    });
  }
  ...
  for (const [name, changes] of [
    ...
    test(`reads no booking when ${name}`, () => {
      expect(bookingIntentFromMetadata(metadata(changes))).toBe(null);
    });
  }
});
