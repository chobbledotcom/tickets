...
- `deno task restore <backup.zip>` - Restore the database named by `DB_URL` / `DB_TOKEN` in `.env` from a local backup. Shows the backup details, asks for typed confirmation, and reports each restore step in the console.
