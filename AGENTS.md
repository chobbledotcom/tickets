...
- `I18N_REPLACEMENTS` - Optional comma-separated `from|to` substring replacements applied to every rendered i18n value, e.g. `ticket|booking,attendee|guest` to rebrand the copy. Matching is case-insensitive and by substring (`ticket|booking` turns `tickets` into `bookings`), and the output copies the source word's capitalisation — `Ticket` → `Booking`, `ticket` → `booking` (only lowercase and title-case are handled, which is all real copy uses). The spec is parsed once at startup into a single regex pass per render (cheap on a cold-booting edge runtime), and replacements never touch the fallback key path returned for a missing translation.
