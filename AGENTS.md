...
- **Trust request key setup**: If the site is processing a request, startup has already validated `DB_ENCRYPTION_KEY`. If it is processing any route other than setup, the atomic setup ceremony has already created the owner and public key. Do not spend CPU cycles or source bytes checking that either key exists in request code, and do not test states that can only be made by corrupting this setup.
...
- `deno task build:desktop` - Build a desktop app for the host platform. The Nix shell runs this one task's `deno desktop` step with the current nixpkgs Deno (`deno-desktop`, version 2.9.0 or newer); all other tasks keep using the pinned Deno 2.5.6 production floor. Desktop transpiles without the newer runtime's checker because `deno task typecheck` remains authoritative at the production floor
- `deno task start:desktop` - Build and open the desktop app on Linux. The Nix shell provides the `start-desktop` launcher and runs the generated binary in a small GTK/WebKit FHS environment, so it works on NixOS without a system-wide `nix-ld` setup. When the development shell's throwaway `DB_URL=:memory:` default is active, the launcher uses `.local-data/desktop.db` and a matching persistent key instead, because separate libsql connections cannot share an in-memory database
...
a run clears out the folders of every earlier run that is no longer going,
including any whose `run.json` is unreadable because it was killed mid-write:
