...
- `BACKUP_MAX_INLINE_SUBREQUESTS` - The per-request edge subrequest cap used to decide whether the automatic pre-migration backup runs inline (default 50, Bunny's hard limit; raise only if your plan allows more). Dumping costs ~one subrequest per table (≈31) plus one per `BACKUP_PAGE_SIZE` rows; the inline backup is skipped when that estimate plus reserved headroom for the migration itself would exceed this — so in practice only small databases back up inline. On a skip the migration still proceeds; take an out-of-band backup with `deno task backup` instead.
