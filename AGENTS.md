...
- **No barrel files**: Don't create barrel/index files that merely `export *` (or re-export) from a folder of real modules, and don't add new entries to the ones we still have. A barrel hides where a symbol actually lives, drags the whole folder into every importer (slower type-checking, worse tree-shaking, accidental cycles), and is exactly the kind of indirection layer we avoid. Import straight from the module that defines the thing — the `#` import map already exposes folders directly (e.g. `#test-utils/session.ts`, not the `#test-utils` barrel). When you touch code that goes through a barrel, migrate it off in passing: repoint each importer at the concrete module and, once a barrel has no importers left, delete it. Treat the existing barrels (`test/test-utils.ts`, `src/features/index.ts`, and friends) as legacy to be dismantled, not a pattern to extend.
