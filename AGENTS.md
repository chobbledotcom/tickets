...
- `SCHEDULED_TASKS_KEY` - Master-only bearer key gating the builder's fleet-walk. Database pruning runs per-request (interval-gated), so this is *not* needed for a site to prune itself — a cron hitting `GET /scheduled` (or any dynamic route) suffices. On a builder (`CAN_BUILD_SITES=true`), an authenticated `POST /scheduled` also pokes the least-recently-pruned built site (an unauthenticated GET that triggers its own per-request prune) and bumps its `last_pruned` stamp, so one cron walks the fleet round-robin. Unset disables the walk (POST is rejected with 401). Deliberately *not* in `HOST_SECRETS`, so it never reaches built sites — there's no fleet-wide rotation to manage.
