...
- `SCHEDULED_TASKS_KEY` - Bearer key for the public `POST /scheduled` cron endpoint, which runs database pruning (`maybeRunPrunes`). Pruning is no longer triggered per-request, so a cron must hit this endpoint (daily is enough; each prune task self-gates to its own interval). When unset the endpoint 404s. On a builder (`CAN_BUILD_SITES=true`), `POST /scheduled?built=true` forwards a prune to the least-recently-pruned built site and bumps its `last_pruned` stamp, so one cron on the master walks every client at a steady pace. The key is copied to built sites via `HOST_SECRETS` so the master can authenticate to them with the same token.
