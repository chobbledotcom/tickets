...
- `I18N_REPLACEMENTS` - Optional comma-separated `from|to` substring replacements applied to every rendered i18n value, e.g. `ticket|booking,attendee|guest` to rebrand the copy. Matching is case-insensitive and by substring (`ticket|booking` turns `tickets` into `bookings`), and the output copies the source word's capitalisation — `Ticket` → `Booking`, `ticket` → `booking` (only lowercase and title-case are handled, which is all real copy uses). URL/route paths are left untouched — a term sitting against a `/` is treated as a path segment, so `attendee|guest` rewrites a link's label but not its `href="/admin/attendees"` target. The spec is parsed once at startup into a single regex pass per render (cheap on a cold-booting edge runtime), and replacements never touch the fallback key returned for a missing translation.
