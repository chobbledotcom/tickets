...
| M4 settled-money diagnosis (was PR 4)   | Part A complete for the current path: #2065 (one durable `payment_charges` authority, one provider-send permit, one owner-recovery lifecycle), #2079 (resumable placeholder completion; each machine declared as a checked spec feeding the `/admin/schema` map), #2084 (the declared seam between the machines, witnessed in every crash test, with a live stored-data check on the map). Whole-checkout diagnosis, stable booking obligations, allocation, and durable Refund All jobs remain later milestones. |
...
| The seam between the machines    | `AuthorityFact`; `JointRowFact`; `ILLEGAL_JOINT_STATES`; `authorityFactOf`; `jointRowFactOf`; `illegalJointReasonOrNull`; `assertJointStateLegal`; `scanJointAnomalies`; `JointAnomaly`                                                                                                                                                                                                                                                                                          | `test/shared/payment/joint-state.test.ts`; `test/shared/db/joint-state-scan.test.ts`; `test/test-utils/joint-state.{ts,test.ts}` (the witness every crash manufacture runs); `test/test-utils/db-fault.ts` (real SQLite faults); `test/features/admin/schema-atlas/server.test.ts`                                                                                                                                                              |
