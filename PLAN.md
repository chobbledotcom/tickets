...
| M4 settled-money diagnosis (was PR 4)   | Part A complete for the current path: #2065 (one durable `payment_charges` authority, one provider-send permit, one owner-recovery lifecycle), #2079 (resumable placeholder completion; each machine declared as a checked spec feeding the `/admin/schema` map), #2084 (the declared seam between the machines, witnessed in every crash test, with a live stored-data check on the map), and #2109 (the SumUp checkout-recovery task on the current path). Whole-checkout diagnosis, stable booking obligations, allocation, and durable Refund All jobs remain later milestones. |
...
10. **Machines, seams, and witnesses ship with the first slice.** Any new
    stateful lifecycle — M5's cases, the M6–M8 aggregate states, M9–M10's
    delivery and site work — lands with its declared machine spec, its seam
    declarations against every machine it shares reality with, witness
    assertions in its crash manufactures, an enumerated flows × crash-windows
    table in the PR description (each window closed by an idempotent-redelivery
    proof or a fault-injected test), and its section on the `/admin/schema` map.
    Retrofitting exactly this onto the refund machines took three large PRs
    (#2065, #2079, #2084); declaring it at design time is the cheap path. The
    method and its reference files are AGENTS.md's "Checked forwards and
    backwards" section.
...
- Retry and replay rules apply only to duplicate callbacks or interrupted work
  ...
  payment behavior.
- A refused checkout's terminal completion records its existing refund authority
  under the same durable session identity, so a later delivery of that session —
  a webhook redelivery, or the buyer opening the success page — short-circuits
  through the same engine and cannot make a second ticket for money already
  returned. No second processed-payment refund marker is added beside the
  canonical identity.
...
the one provider-refund authority. Its machines are declared as checked specs,
the seam between them is a declared illegal-combinations table witnessed by
every crash test, and the `/admin/schema` map renders both and checks the stored
data against them — the reference for delivery rule 10. It does not build a
whole-checkout observation, allocate one charge across stable booking
obligations, or write the other dormant aggregate tables. The former
`PR4_PLAN.md` has been retired; this as-built map and the fault ledger preserve
the useful decisions without leaving a second planning document to drift from
the code.
...
| Provider reads and attempts      | `ProviderRead`; `providerFailure`; `RefundAttemptResult`; `RefundRequest`; `refundOutcomeAfterReread`; `refundWithOneReread`; `SquarePaymentStatus`; `isSquarePaymentStatus`                                                                                                                                                                                                                                                                                                     | `test/shared/payment/{provider-failures,refund-attempt}.test.ts`; `test/shared/stripe-provider/{outcomes,refund-outcomes}.test.ts`; `test/shared/square/payment-outcomes/refund.test.ts`; `test/shared/sumup/{transaction,provider-money}.test.ts`; `test/shared/square-provider/{metadata,webhook-fields}.test.ts`                                                                                                                             |
...
| Bounded command support          | `refundReadinessSubrequestCost`; `refundPreparedSubrequestCost`; `REFUND_SETTLEMENT_SUBREQUEST_RESERVE`; `REFUND_LEDGER_SUBREQUEST_RESERVE`; `REFUND_OBSERVED_AUTHORITY_DATABASE_CALLS`; `REFUND_ACTIVE_AUTHORITY_DATABASE_CALLS`; `REFUND_TERMINAL_AUTHORITY_DATABASE_CALLS`; `REFUND_RETRY_HEADROOM_DATABASE_CALLS`; `REFUND_RESULT_DATABASE_RESERVE`; `withSubrequestReserve`; `withDeferredErrorReports`; `getRefundCandidates`; `getRefundAllSummary`; `loadRefundAllBatch` | `test/features/admin/refunds/{budget,claim-settlement,candidates}.test.ts`; `test/features/admin/refunds/provider/batch/budget.test.ts`; `test/features/admin/refunds/dispatch/budget-lifecycle.test.ts`; `test/features/admin/refunds/readiness-run/refresh-budget.test.ts`; `test/shared/provider-refunds/{budget,send}.test.ts`; `test/shared/subrequest-budget.test.ts`; `test/integration/logger/log-error.test.ts`                        |
...
| Declared machines and their map  | `MachineNode`; `MachineEvent`; `MachineMoves`; `movesIn`; `machineRep`; `factsAndStart`; `factsFromNode`; `nodeIdsWhere`; `derivedNodeIds`; `atlasMachineFrom`; `ROW_NODES`; `ROW_EVENTS`; `ROW_MOVES`; `rowNodeOf`; `rowWorkMirrorSql`; `REFUND_NODES`; `REFUND_EVENTS`; `REFUND_MOVES`; `refundNodeOf`; `refundNodeSendsMoney`; `REVIEW_NODES`; `reviewNodeOf`; `SCHEMA_ATLAS_MACHINES`                                                                                        | `test/shared/payment/{row,refund,review}-machine-spec.test.ts`; `test/shared/payment/{row,refund}-machine-spec/graph.test.ts`; `test/test-utils/machine-graph.{ts,test.ts}`; `test/ui/templates/admin/schema-atlas.test.tsx`; `test/integration/refund-authority-architecture.test.ts`                                                                                                                                                          |
| Machine seams and stored checks  | `AuthorityFact`; `JointRowFact`; `ILLEGAL_JOINT_STATES`; `authorityFactOf`; `jointRowFactOf`; `illegalJointReasonOrNull`; `assertJointStateLegal`; `scanSchemaAnomalies`; `SchemaAnomaly`; `listUnansweredSumupMoney`                                                                                                                                                                                                                                                            | `test/shared/payment/joint-state.test.ts`; `test/shared/db/schema-anomaly-scan.test.ts`; `test/test-utils/joint-state.{ts,test.ts}` (the witness every crash manufacture runs); `test/test-utils/db-fault.ts` (real SQLite faults); `test/features/admin/schema-atlas/server.test.ts`                                                                                                                                                           |
...
- **Extend one callback/admin lifecycle.** M4 already persists the exact charge,
  ...
  durable, reachable exits in Refund recovery. The missed-SumUp-checkout task
  already runs on the current path as `sumup_checkout_recovery`; M7 adopts that
  task instead of building it again. M7 attaches scheduled refund retries and
  the original checkout's handled marker to this same lifecycle. It must not
  reinterpret a placeholder's `legacy:` anchor as the original checkout, a sent
  attempt, provider completion, or durable replay marker.
