...
- `BOTPOISON_PUBLIC_KEY` - Botpoison public key (sent to the browser). Set together with `BOTPOISON_SECRET_KEY` to enable the public contact form; the owner must also enable it under Site → Contact and set a business email.
- `BOTPOISON_SECRET_KEY` - Botpoison secret key. Used server-side to verify contact form submissions before emailing the business address. Never sent to the browser.
